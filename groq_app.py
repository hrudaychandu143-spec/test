...
from lang chain_core.prompts import ChatPromptTemplate
...
st.image("OIP.jpj")
st.title("Dynamic RAG with Groq, FAISS, and Llamаз")
...
if prompt_input := st.chat_input("Ask a question about your documents..."):
    if st.session_state.vector is not None:
        ...
        with st.Inner("Thinking..."):
            document_chain = create_stuff_documents_chain(llm, prompt)
            retriever st.session_state.vector.as_retriever()
            retrieval_chain = create_retrieval_chain(retriever, document_chain)
            start time.process_time()
            response=retrival_chain.invoke({"input":prompt_input})
